...
api.createSlideshow = function (container, source) {
  events.emit('createSlideshow', container, source);
};

api.resizeSlideshow = function () {
  events.emit('resizeSlideshow');
};
...
function Controller (container, slideshow) {
...
function Dispatcher (container) {
  ...
  mapKeys(container);
  mapTouches(container);
  mapWheel(container);
}

function mapHash (container) {
  container.addEventListener('hashchange', navigate);
  navigate();
...
function SlideshowView (slideshow, container) {
  var self = this;

  mapEvents(self, container);

  self.element = createSlideshowElement();
  ...
  self.overlayView = new OverlayView(self.element);
}

function createSlideshowElement () {
  var element = document.createElement('div')
  element.className = 'remark-slideshow';
  return element;
}
...
function mapEvents (slideshowView, container) {
  ...
  events.on('resizeSlideshow', onResize);
...
function setClassFromProperties (element, properties) {
  ...
  element.className = ['remark-slide-content'].concat(classes).join(' ');
}
...
require.define("/src/remark.js",function(require,module,exports,__dirname,__filename,process,global){var EventEmitter = require('events').EventEmitter 
  , utils = require('./remark/utils')
...
events.on('createSlideshow', createSlideshow);

function onLoad () {
  styleDocument();
  createStandaloneSlideshow();
...
function createStandaloneSlideshow () {
  var sourceElement = document.getElementById('source');

  if (sourceElement) {
    sourceElement.style.display = 'none';

    createSlideshow(document.body, sourceElement.innerHTML);
  }
}

function createSlideshow (container, source) {
  var containerProxy = createContainerProxy(container)
    , slideshow
    , slideshowView
    , controller
    , dispatcher
  ...
  slideshowView = new SlideshowView(slideshow, containerProxy);
  controller = new Controller(container, slideshow);
...
function styleContainer (container) {
  ...
  if (container === document.body) {
    ...
    html.className = html.className + ' remark-container';
  }
  else {
    container.style.position = 'absolute';
  }

  container.className = container.className + ' remark-container';
}

function createContainerProxy (container) {
  var proxy = new EventEmitter();

  if (container === document.body) {
    proxy.embedded = false;

    proxyEvent(window, proxy, 'resize');
    proxyEvent(window, proxy, 'hashchange');
    proxyEvent(window, proxy, 'keydown');
    proxyEvent(window, proxy, 'keypress');
    proxyEvent(document, proxy, 'touchstart');
    proxyEvent(document, proxy, 'touchmove');
    proxyEvent(document, proxy, 'touchend');
    proxyEvent(window, proxy, 'mousewheel');

    proxy.getWidth = function () { return window.innerWidth; };
    proxy.getHeight = function () { return window.innerHeight; };
  }
  else {
    proxy.embedded = true;

    proxyEvent(container, proxy, 'keydown');
    proxyEvent(container, proxy, 'keypress');
    proxyEvent(container, proxy, 'touchstart');
    proxyEvent(container, proxy, 'touchmove');
    proxyEvent(container, proxy, 'touchend');
    proxyEvent(container, proxy, 'mousewheel');

    proxy.getWidth = function () { return container.clientWidth; };
    proxy.getHeight = function () { return container.clientHeight; };

    container.tabIndex = -1;
  }

  proxy.appendChild = function (child) { container.appendChild(child); };

  return proxy;
}

function proxyEvent (source, target, eventName) {
  source.addEventListener(eventName, function () {
    target.emit.apply(target, Array.prototype.slice.call(arguments));
  });
}
